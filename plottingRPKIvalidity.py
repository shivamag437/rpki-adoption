...
def main():
	...
	ax0.scatter(rpki_invalids, pos,s = 8,  alpha = 0.6, label ='RV/RIS feeder AS')
	# ax0.scatter(v6_invalidLength+v6_invalidASN, v6_pos,  alpha = 0.6)
	# ax0.plot([0,1000000*0.16*v4ROV_cut], [0,1000000], linewidth = 1, color = 'C2', linestyle = '--', label='RPKI filtering threshold')
	# ax0.plot([v4ROV_cut *0.16*v4_ff_cut,1000000*0.16*v4ROV_cut], [v4_ff_cut,1000000], linewidth = 1, color = 'C2', linestyle = '--', label='RPKI filtering cut off')
	ax0.axvline(x=ROV_cut, ymin=float(ff_cut)/ylim*1.05, linewidth = 1, color='C2', linestyle = '--', label= 'RPKI filtering threshold')
	ax0.axhline(y=ff_cut, linewidth = 1, color = 'C1', linestyle = '--', label = 'Full feeder threshold')
	# ax0.set(ylabel = '# Unique Prefix Origin pairs', xlabel='# RPKI Invalid IPv6 Prefix Origin pairs')
	# ax0.set(ylabel = '# Unique prefix-origin pairs', xlabel='# RPKI invalid IP'+ipv+' prefix-origin pairs')
	ax0.set(ylabel = '# Unique routes', xlabel='# RPKI-invalid IP'+ipv+' routes')
	ax0.set_ylim(top = ylim)
	ax0.set_xlim(right = xlim)
	...
	plt.savefig('/Users/ctestart/Dropbox (MIT)/RA/BGP/Filtering/Figures/'+graph_name+graph_type+sq+'_'+date+'.png', dpi = 300)
	plt.show()
